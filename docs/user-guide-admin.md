...
By default, the alert email goes to the **failing invoice's billing-company `accountsEmail`** — the field set on the billing company that owns the invoice (Settings → Companies → edit → "Accounts email"). If that field is blank for a billing company, no email is sent for invoices owned by that billing company, and Telegram becomes the only channel.

This is fine for a single-billing-co setup but has two visibility problems for multi-billing-co operators:
1. You only see alerts for invoices whose billing company has *your* address as the accounts email.
2. If a billing company has no accounts email, you don't get alerted at all.

To always receive every alert regardless of which billing company is affected, set the env var:

```
NOTIFICATION_EMAILS=alice@you.com,bob@you.com
```

Behavior:
- **Unset (default)** — only the billing-company `accountsEmail` is used.
- **Set** — every address in the list receives the alert *in addition to* the billing-company `accountsEmail`. Multiple recipients are delivered in a single Resend API call (one quota slot).
- Addresses are trimmed, lowercased, and deduped against the billing-company address — adding the same address that's already on the billing company won't double-send.

This is an admin-ops setting (which humans should be paged), not a per-tenant setting, so it lives in env rather than the UI. To change it, edit `.env` and `docker compose up -d backend` (NOT `restart` — that doesn't re-read .env).
...
- **`NOTIFICATION_EMAILS`** — if you run multiple billing companies (or want a personal copy of every alert), set this to a comma-separated list of addresses. Every entry receives the alert in addition to the billing-company `accountsEmail`.
