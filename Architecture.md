...
- `telegram` — Telegraf-based bot + allowlist sub-module. **(v0.10; allowlist made editable v0.11.3)** Every incoming update is resolved Telegram handle → `TelegramAllowlist` → linked `User` (via the new `userId` FK); the resolved user is attached to `ctx.state.user` and every command runs through `RolesService.hasCapability` before any side effect. Allowlist CRUD: `GET/POST /telegram/allowlist`, `PATCH /telegram/allowlist/:id` (full edit — username/linked-user/botName/botToken/note), `DELETE /telegram/allowlist/:id`. All admin-only. Surfaces: `/start`, `/help`, `/cancel`, `/newtask <title>` (or `/newtask` alone → force-reply prompt), `/tasks` (lists open tasks with inline keyboards `[ ✓ Done ] [ ✏️ Edit ] [ 🗑 Delete ]` per row — Delete only rendered when `action.delete` is granted). Callback handlers: `task:done:<id>`, `task:edit:<id>`, `task:delete:<id>` (followed by a Yes/Cancel confirm step → `task:delete:confirm:<id>`). The edit + new-task flows use an in-memory `Map<chatId, pendingFlow>` to capture the next free-text message. DTO validation reuses the same `CreateTaskDto` as the HTTP API — the bot never re-implements validation. Webhook mode when `TELEGRAM_WEBHOOK_DOMAIN` is set; long-poll otherwise. `notify(text)` broadcasts to every chat that has ever run `/start`.
- `tax-types`, `mail-configuration`, `invoice-templates`, `email-templates`, `preferences` — settings. **(v0.12.0)** Preferences now also stores `defaultInvoiceTerms` (singleton system-wide default Terms text). Read via `GET /preferences/terms`, written via `PUT /preferences/terms { defaultInvoiceTerms }`. Both endpoints are gated by the new `settings.terms` capability — ADMIN + BOOKKEEPER + API_USER can edit, ACCOUNTANT cannot. `InvoicesService.create` and `RecurringService.create` substitute the configured default whenever the inbound payload omits `terms` (explicit empty string from the client is preserved).
...
| `POST` | `/invoices/trash/purge-all` | (v0.12.0) Hard-deletes every soft-deleted invoice in one transaction (Allocations linked to those invoices are deleted first to satisfy the RESTRICT FK; `AllocationEvent` snapshot rows survive because their `invoiceId` is a plain string, not a FK). Returns `{ purged: <count> }`. Admin-only. Surfaced as "Empty Recycle Bin" on `/settings/data-retention`. Replaces the v0.10-and-earlier automatic 30-day `sweepTrash` call from `RecurringProcessor` — that auto-purge was removed in v0.12.0 so trashed invoices live indefinitely. |
| `GET` | `/preferences/terms` | (v0.12.0) Returns `{ defaultInvoiceTerms: string \| null }`. Gated by `settings.terms`. |
| `PUT` | `/preferences/terms` | (v0.12.0) Body: `{ defaultInvoiceTerms?: string }`. Empty string clears the default. Gated by `settings.terms`. |
