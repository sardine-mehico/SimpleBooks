...
- **Auto-alias pass is idempotent and runs on every CSV import.** Goes after the optional rule engine pass. Builds the match index once from all active tags (`tag.name` + each `tag.aliases[]`), regex `(?:^|[^a-z0-9])<pattern>(?=[^a-z0-9]|$)` case-insensitive, longest-pattern-first. Insert via `transactionTag.createMany({ skipDuplicates: true })` so re-runs don't double-tag. The two "Re-apply" buttons on `/settings/tags` call the same pass — per-tag (`POST /tags/:id/auto-apply`) and global (`POST /tags/auto-apply`). Manual edits via `PATCH /transactions/:id/tags` replace the tag set wholesale (source=USER); they're not affected by the auto-alias pass on later imports unless the user explicitly re-applies.
- **AI Rule Drafter is DISABLED** as of 2026-05-28. The previous clustering signal leaned on `vendor.name` patterns; with Vendor gone, `AiRuleDrafterService.mine()` early-returns `{ drafted: 0, disabled: true }`. Helpers (`clusterKey`, `computeClusterHash`) stay in place. Re-enable by introducing a new signal — description-prefix mining or token-frequency over `Transaction.description` is the obvious next step. The auto-memory entry `project_ai_rule_drafter_pending.md` tracks this.
