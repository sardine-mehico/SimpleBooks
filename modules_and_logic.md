...
- **Settings → Audit Log** (`/settings/audit`) — append-only log of logins, role changes, user CRUD, deletes (auto-captured), retention purges. Filter by action / date range / actor. Table loads up to 500 rows, is shown 50/page in a sticky-header scroll container (v0.10.3).
