...
- **Settings → Data Retention** (`/settings/data-retention`) — per-table count + oldest entry. Two paths:
  ...
  - **Auto-purge daily** (v0.10.3) — flip the Auto-purge switch and pick a cutoff (`7d`/`30d`/`90d`/`1y`). A nightly BullMQ sweep at 03:15 deletes anything older than the chosen cutoff for every enabled policy. `RetentionPolicy.lastRunAt` is rendered under the switch so admins can see the last successful run. `all` is intentionally not offered for auto — only manual.
