...
- **Menu** button (`lucide-react` `Menu` icon followed by the literal text "Menu") opens a Radix `DropdownMenu` with all per-invoice actions:
  - **Clone** — `POST /invoices/:id/clone`. Duplicates header + line items into a new `DRAFT`. Fresh `invoiceNumber` (MAX + 1); `invoiceDate` resets to today; `dueDate` is computed at clone time from the customer's `paymentTerms` (via `paymentTermsOffsetDays`) — same logic the New Invoice form runs on customer change, and the same logic the recurring sweep uses (v0.11.4; previously left null which left the form blank because the form's recompute effect only fires after mount). Send-tracking columns (`sendAttempts`, `sendError`, `lastSendAt`) are not copied. Navigates to the clone's edit page.
...
### List page — `/recurring`
- **Columns:** Schedule Name · Customer · Recurring Schedule · Next Run · Amount (tax-inclusive total: `Σ unitPrice × (1 + taxRate/100)`, client-computed; v0.12.0 — previously the column summed only `unitPrice` and excluded tax, so the displayed amount disagreed with what the processor actually stamped onto the generated `Invoice.totalAmount`) · Sending Option · Active.
...
- **Settings → Audit Log** (`/settings/audit`) — append-only log of logins, role changes, user CRUD, deletes (auto-captured), retention purges. Filter by action / date range / actor. Table loads up to 500 rows, is shown **100/page** in a sticky-header scroll container (v0.10.3; page size bumped to 100 in v0.11.3 to match the other list pages).
- **Settings → Data Retention** (`/settings/data-retention`) — per-table count + oldest entry. Three sections:
  - **Manual** — pick a cutoff from the right-hand dropdown (`7d`/`30d`/`90d`/`1y`/`all`, default `1y`) and click "Purge now". One-shot, audited as the actor.
  - **Auto-purge at selected interval** (v0.10.3, renamed v0.11.2) — flip the Auto-purge switch and pick a cutoff (`7d`/`30d`/`90d`/`1y`). The cutoff defines a rolling retention window: a nightly BullMQ sweep at 03:15 checks every enabled policy and deletes only the rows that are now older than the cutoff. Rows newer than the cutoff stay put — most nights the sweep is a no-op for that table. `RetentionPolicy.lastRunAt` is rendered under the switch so admins can see the last successful run. `all` is intentionally not offered for auto — only manual.
  - **Recycle bin** (v0.12.0) — section at the bottom shows the current count of soft-deleted invoices and an "Empty Recycle Bin" button. Hits `POST /invoices/trash/purge-all` which hard-deletes every soft-deleted invoice in a single transaction (Allocations linked to those invoices are removed first; `AllocationEvent` rows are kept since they're append-only string snapshots, not FKs). Admin-only. **There is no automatic invoice purge anymore** — the previous 30-day `sweepTrash` call inside `RecurringProcessor.process()` was removed in v0.12.0, so trashed invoices live indefinitely until an admin clicks Empty Recycle Bin.
  - Covered log tables: `AuditLog`, `TransactionImport`, `AllocationEvent`, `CategorisationEvent` (with AI-training warning — recent events feed Phase C's few-shot examples), `AiCall`, `Session`. Sessions are also auto-pruned hourly by `AuthService` independent of any policy.
- **Settings → Terms** (`/settings/terms`) — **(v0.12.0)** singleton textarea storing the default Terms text. `Preferences.defaultInvoiceTerms` (nullable). On every new invoice (`POST /invoices`) and every new recurring rule (`POST /recurring`), if the caller omits `terms` from the payload, the service substitutes the stored default; explicit empty string from the client means "no terms" and is preserved as-is. Line breaks (`\n`) survive end-to-end through the form textarea, the React-PDF templates (which already split paragraphs on `\n`), and the public invoice page. Gated by the `settings.terms` capability: ADMIN + BOOKKEEPER + API_USER can edit; ACCOUNTANT cannot.
