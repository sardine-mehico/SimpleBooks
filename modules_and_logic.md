...
- **Settings → Data Retention** (`/settings/data-retention`) — per-table count + oldest entry. Two paths:
  ...
  - Covered tables: `AuditLog`, `TransactionImport`, `AllocationEvent`, `CategorisationEvent` (with AI-training warning — recent events feed Phase C's few-shot examples), `AiCall`, `Session`. Sessions are also auto-pruned hourly by `AuthService` independent of any policy.
