...
//
// Clustering signal: the first ≤2 alphabetic-only tokens of the transaction
// description, before any token containing a digit. This strips merchant-
// location suffixes like "1234 SUBIACO" and reduces e.g.
//   "WOOLWORTHS 1234 PERTH" / "WOOLWORTHS METRO ATM" / "WOOLWORTHS 5678 SUBIACO"
// to the shared key "WOOLWORTHS". A cluster is a (key, categoryId) pair with
// ≥ threshold supporting events and ≥ AGREEMENT_RATIO category agreement.
//
// (2026-05-28) The clustering was already description-based when the vendor
// concept existed; only the LLM prompt referenced the VENDOR field. The
// drafter was briefly disabled during the Vendor->Tags migration out of
// caution and re-enabled here once the prompt + schema were stripped of
// VENDOR references.
...
ACCOUNT EQUALS or IN is appropriate when the pattern is scoped to specific bank accounts.
...
Output strict JSON matching the schema.`;
        ...
        properties: {
          ...
          conditions: {
            ...
            items: {
              ...
              properties: {
                field: { enum: ['DESCRIPTION', 'AMOUNT', 'ACCOUNT'] },
  ...
  private validateRule(r: DraftRuleLlmResponse, cluster: Cluster, fallbackCategoryName: string) {
    ...
    for (const c of r.conditions) {
      if (!['DESCRIPTION', 'AMOUNT', 'ACCOUNT'].includes(c.field)) return null;
