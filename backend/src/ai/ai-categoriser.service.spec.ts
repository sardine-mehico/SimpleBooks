...
describe('buildCategoriseUserPrompt', () => {
  it('renders leaves with parent breadcrumbs', () => {
    const out = buildCategoriseUserPrompt({
      ...
      fewShots: [],
      tx: { date: '2026-05-25', amount: '-12.50', description: 'TEST', accountName: 'Cheque' },
    });
...
describe('AiCategoriserService.suggest', () => {
  it('records providerId on the AI_DRAFT CategorisationEvent', async () => {
    const created: any[] = [];
    const prisma: any = {
      transaction: { findUnique: jest.fn().mockResolvedValue({ id: 't1', date: new Date('2026-05-25'), amount: '-12.50', description: 'X', account: { id: 'a1', name: 'Cheque' } }) },
    ...
    const ai: any = {
      complete: jest.fn().mockResolvedValue({ ok: true, data: { categoryId: 'cat1', confidence: 'high', reasoning: 'ok' }, providerId: 'prov-1', attempts: 1, promptTokens: 10, completionTokens: 5 }),
    };
