...
// Reuses the Inter latin subset already registered for grey-1. `Font.register`
// is idempotent (last call wins per family), so calling it again here means
// orange-1 still works even when rendered first in a session.
const FONT_DIR =
...
const COLOR = {
  pageBg: '#fceee5',     // warm cream page tint
  brand: '#c4451c',      // rust / red-orange for headings + badges
  brandOnDark: '#ffffff',
  ink: '#1a1a1a',
  inkSoft: '#7c8285',    // muted slate for secondary lines
  border: '#d8d2cd',     // thin neutral border (warm-ish)
};

// Match the grey-1 sizing knobs so iteration across templates feels symmetric.
const FS = 0.8;
const RP = 0.75;
const LH = 0.75;
...
const styles = StyleSheet.create({
  ...
  // 10pt inset from the page content edge (header, line rows, totals all
  // share the same right padding so the column stays aligned).
  tableHeader: {
    flexDirection: 'row',
    paddingTop: 8,
    paddingBottom: 8,
  ...
  // Right padding zeroed so the totals values align with the line-item
  // amounts above (which sit flush right against the page-content edge).
  totalsBox: { width: 143 },
  totalRow: {
    flexDirection: 'row',
    justifyContent: 'space-between',
  ...
  // The "PAYMENT DETAILS" / "Terms" banners are tiny rust-coloured pills
  // sized to their text — left-aligned so the body content beneath them
  // reads as one column.
  bannerBox: {
...
function deriveTaxLabelTotals(invoice: PdfTemplateProps['invoice']): string {
  ...
  // Strip a trailing " 10%" off the name for the totals label since GST is
  // a category and the rate already lives elsewhere; keeps the totals box
  // visually tidy (matches the mockup's "GST" line).
  return names[0]!.replace(/\s*\d+(\.\d+)?%$/, '');
}

export default function Orange1Template({ invoice, company, customer }: PdfTemplateProps) {
  ...
  return (
    <Document>
      <Page size="A4" style={styles.page}>
        {/* Header — title on the left, company block on the right. */}
        ...
        {/* Middle row — Bill To on the left, meta table on the right. */}
        ...
        {/* Line-item table — top/bottom rules around the header, faint
            dividers between body rows, no surrounding box. */}
