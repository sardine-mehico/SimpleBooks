...
// brown-black uses Lora throughout — a transitional serif that reads
// elegantly at small body sizes and looks distinguished in caps for the
// "TAX INVOICE" title and the items header. Two weights (400 + 700)
// loaded; both Latin subset WOFFs.
const LORA_DIR =
  path.dirname(require.resolve('@fontsource/lora/package.json')) + '/files';
Font.register({
  family: 'Lora',
  fonts: [
    { src: `${LORA_DIR}/lora-latin-400-normal.woff`, fontWeight: 400 },
    { src: `${LORA_DIR}/lora-latin-700-normal.woff`, fontWeight: 700 },
  ],
});

const COLOR = {
  pageBg: '#f2efe9',          // warm beige, another +20% lightening pass
  brand: '#b3541a',           // dark orange — replaces the prior chocolate
                              //   brown on the TAX INVOICE title, header
                              //   rule, and email link
  blackBand: '#1a1a1a',       // black band for items header + Total row
  ink: '#1a1a1a',
  // Darker warm slate so address / bill-to / payment / terms text reads
  // more clearly against the lighter page tint.
  inkSoft: '#4a4540',
  divider: '#bcb3a5',         // muted warm grey divider
  onDark: '#ffffff',
};
...
const styles = StyleSheet.create({
  ...
  // Meta column right-aligned. -35% width (was 200 → 130).
  metaRows: { width: 130 },
  metaRow: {
    flexDirection: 'row',
    justifyContent: 'space-between',
  ...
  metaLabel: { fontSize: 10, fontWeight: 700, color: COLOR.ink },
  metaValue: { fontSize: 10, fontWeight: 700, color: COLOR.ink, textAlign: 'right' },

  // Items table — black band, white DESCRIPTION + brown AMOUNT, then
  // plain rows separated by thin warm-grey dividers.
  tableHeader: {
    flexDirection: 'row',
    backgroundColor: COLOR.blackBand,
    paddingTop: 10,
    paddingBottom: 10,
    paddingLeft: 16,
    paddingRight: 16,
  },
  th: { fontSize: 10, fontWeight: 700, letterSpacing: 1.2, textTransform: 'uppercase' },
  thDescription: { flex: 4, color: COLOR.onDark },
  thAmount: { flex: 1.4, textAlign: 'right', color: COLOR.onDark },
  tableRow: {
    flexDirection: 'row',
    paddingTop: 11 * RP,
    paddingBottom: 11 * RP,
  ...
  totalLabel: { fontSize: 10, fontWeight: 700, color: COLOR.ink },
  totalValue: { fontSize: 10, fontWeight: 700, color: COLOR.ink },
  // Final Total row sits on the same black band as the items header,
  // with the value picked out in brown to mirror "TAX INVOICE" above.
  grandTotalRow: {
    flexDirection: 'row',
    justifyContent: 'space-between',
    alignItems: 'baseline',
  ...
  // Footer — thin grey divider then a vertical stack: PAYMENT DETAILS first,
  // then TERMS below at full width.
  footer: {
    marginTop: 36,
    paddingTop: 22,
    borderTopWidth: 0.5,
  ...
  paymentLine: { fontSize: 10, color: COLOR.inkSoft, marginBottom: 3 },
  termsLine: { fontSize: 10, color: COLOR.inkSoft, marginBottom: 3, lineHeight: 1.5 * LH },
});
...
export default function BrownBlackTemplate({ invoice, company, customer }: PdfTemplateProps) {
